...
class Handler(BaseHandler):
    ...
    def on_start(self):
        ...
        jdata = '{"iMddid":"12719","iPage":"1","iTagId":"0","sAct":"KMdd_StructWebAjax|GetPoisByTag"}';
    ...
    def index_page(self, response):
        ...
        jdata = '{"iMddid":"12719","iPage":"'+ipage+'","iTagId":"0","sAct":"KMdd_StructWebAjax|GetPoisByTag"}';
        param = json.loads( jm( jdata ) )
        url = "https://www.mafengwo.cn/ajax/router.php"
        self.crawl(url,params=param,callback=self.index_page)
#        self.crawl(url,params=param,callback=self.index_page,proxy=random_proxy())
    ...
    def detail_page_poi(self, response):
        ...
        self.crawl(poiSubPoiApi, callback=self.index_page_subpoi,headers=gen_headers(response.url),save={'pid':poi})
    ...
    def index_page_subpoi(self, response):
        print (response.json)
        pid = response.save['pid']

        result = response.json["data"]
        if result["html"] =="" :  return
        list_doc = pq(result["html"])
        for d in list_doc('a[href^="/poi"]').items():   
            self.crawl("https://www.mafengwo.cn"+d.attr.href, callback=self.detail_page_subpoi,save={'go':d.find('em').text(),'pid':pid})
#            self.crawl("https://www.mafengwo.cn"+d.attr.href, callback=self.detail_page_subpoi,save={'go':d.find('em').text(),'pid':pid},proxy=random_proxy())
        ...
        if page_json['hasMore'] :
            ...
            self.crawl(poiSubPoiApi, callback=self.index_page_subpoi,headers=gen_headers(refer),save={'pid':poi})
    ...
    def on_result(self, result):
        ...
        poi = db["mafengwo"]
        if poi.find_one({'poi': result.get('poi')}):
            poi.update({'poi': result.get('poi')}, {'$set': result})
        else:
            poi.insert_one(result)
